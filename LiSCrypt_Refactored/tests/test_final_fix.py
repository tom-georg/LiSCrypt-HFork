...
def test_decrypt_with_original_method():
    """Test decryption using the exact original method"""
    ...
    # Step 1: Calculate SHA512 of the password (as original app does)
    sha512_hash = hashlib.sha512(password.encode('utf-8')).digest()
    print(f"SHA512 of password: {sha512_hash.hex()}")
    
    # Step 2: Use SHA512 hash as input to Scrypt to get master key
    kdf = Scrypt(
    ...
    master_key = kdf.derive(sha512_hash)
    print(f"Master key: {master_key.hex()}")
    
    # Step 3: Derive encryption key using HKDF from master key
    hkdf_key = HKDFExpand(
    ...
    # Step 4: Try to decrypt using header nonce
    try:
        # Get encrypted content
        encrypted_with_tag = data[header_length:]
        
        # The format is: encrypted_data + tag_length(4 bytes) + tag
        # Extract tag length (last 4 bytes before tag)
        tag_length_bytes = encrypted_with_tag[-20:-16]  # 4 bytes before the last 16
        tag_length = struct.unpack('>I', tag_length_bytes)[0]
        print(f"Tag length from file: {tag_length}")
        
        # Extract tag (last tag_length bytes)
        tag = encrypted_with_tag[-tag_length:]
        
        # Extract encrypted data (everything except tag length and tag)
        encrypted_data = encrypted_with_tag[:-4-tag_length]
...
if __name__ == '__main__':
    test_decrypt_with_original_method()
